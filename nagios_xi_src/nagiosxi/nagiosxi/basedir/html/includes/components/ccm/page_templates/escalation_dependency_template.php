    ...
    <div id='tab1'>
        <div class="leftBox">
            <div class="ccm-row neptune-spacer">
                ...
                <input name="tfConfigName" type="text" id="tfConfigName" class='required form-control fc-fl' value="<?php (!empty($FIELDS['config_name'])) ? val(encode_form_val($FIELDS['config_name'])) : "'"; ?>">
            </div>
            <?php
            // For escalations only
            if (strpos($this->exactType, 'escalation')) {
                $options = (!empty($FIELDS['escalation_options'])) ? explode(',', $FIELDS['escalation_options']) : array();
            ?>
            <div class="ccm-row">
                <label><?php echo _("First / Last Notification"); ?> <span class="req">*</span></label>
                <input type='text' class='form-control required' style="width: 49%; margin-right: 2%; display: inline-block;" name='tfFirstNotif' id='tfFirstNotif' placeholder="<?php echo _('First'); ?>" value="<?php (!empty($FIELDS['first_notification'])) ? val(encode_form_val($FIELDS['first_notification'])) : ""; ?>"><input type='text' class='form-control required' style="width: 49%; display: inline-block;" name='tfLastNotif' id='tfLastNotif' placeholder="<?php echo _('Last'); ?>" value="<?php (!empty($FIELDS['last_notification'])) ? val(encode_form_val($FIELDS['last_notification'])) : ""; ?>">
            </div>
            <div class="ccm-row">
                <label for='tfNotifInterval'><?php echo _("Notification Interval"); ?> <span class="req">*</span></label>
                <div class="input-group">
                    <input type='text' class='form-control required' name='tfNotifInterval' id='tfNotifInterval' size='2' value="<?php (!empty($FIELDS['notification_interval'])) ? val(encode_form_val($FIELDS['notification_interval'])) : ""; ?>">
            ...
            if (strpos($this->exactType, 'dependency')) {
                ...
                if (!empty($FIELDS['inherits_parent']) && $FIELDS['inherits_parent'] == "") { $FIELDS['inherits_parent'] = 1; }
                $exfc = (!empty($FIELDS['execution_failure_criteria'])) ? explode(',', $FIELDS['execution_failure_criteria']) : array();
                $nfc = (!empty($FIELDS['notification_failure_criteria'])) ? explode(',', $FIELDS['notification_failure_criteria']) : array();
            ?>
            <div class="ccm-row oneline neptune-spacer">
                <div class="checkbox">
                    <input type="checkbox" value="1" class="neptune-ccm-checkbox" name="chbInherit" id="chbInherit" <?php check('inherits_parent', 1); ?>>
                    <label class="neptune-checkbox-label" for="chbInherit">
                         <?php echo _("Inherit from parents"); ?>
                    </label>
                </div>
            </div>
            <div class="ccm-row neptune-spacer">
