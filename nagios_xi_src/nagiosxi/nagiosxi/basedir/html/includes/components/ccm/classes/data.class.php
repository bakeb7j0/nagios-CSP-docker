...
class Data
{
    ...
    /**
     * Deletes an object and all of its relations
     *
    ...
    function purgeRelations($intDataId, $strTableName, $arrRelations, &$intDataCount=1, &$arrData=array())
    {
        ...
        foreach ($arrRelations as $rel) {
            ...
            if ($arrFlags[3] == 1) { //1:n
                ...
                $specialRelationTables = array('tbl_lnkServiceToHost', 'tbl_lnkHostescalationToHost', 'tbl_lnkMrtgfileToHost');
    ...
    function writeLog($str_message, $audit_type=AUDITLOGTYPE_MODIFY)
    {
        ...
        $utf8StrMessage = mb_convert_encoding($str_message, 'UTF-8', 'ISO-8859-1');
        $booReturn = $ccm->db->insertData("INSERT INTO `tbl_logbook` SET `user`='".$ccm->db->escape_string($strUserName)."',`time`=NOW(), `ipadress`='".$ccm->db->escape_string($address)."', `domain`='".$ccm->db->escape_string($strDomain)."', `entry`='".$ccm->db->escape_string($utf8StrMessage)."'");
        ...
        switch ($strTable) {
            ...
            case "tbl_mrtgfile":
                $arrRelations[] = array('tableName' => "tbl_host",
                                        'fieldName' => "host_name",
                                        'target' => "host_name",
                                        'linktable' => "tbl_lnkMrtgfileToHost",
                                        'type' => 2);
        ...
        switch ($strTable) {
            ...
            case "tbl_host":
                ...
                $arrRelations[] = array('tableName' => "tbl_lnkServiceToHost",
                                  ...
                                  'targetKey' => "config_name,service_description",
                                  'flags'   => "1,1,0,1");
                $arrRelations[] = array('tableName' => "tbl_lnkMrtgfileToHost",
                                  'fieldName' => "idSlave",
                                  'target'  => "tbl_mrtgfile",
                                  'targetKey' => "file_name",
                                  'flags'   => "1,1,0,1");
