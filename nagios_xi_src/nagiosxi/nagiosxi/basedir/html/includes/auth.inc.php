 ...
 * @param   bool    $v2_api     True to indicate use of v2 API calls coming from Neptune
 */
function check_authentication($redirect = true, $backend = false, $apikey = false, $v2_api = false)
{
    global $request;
...
function is_auth_token_authenticated()
{
    ...
    if (user_is_valid_auth_token($auth_token)) {
        ...
        if (!user_account_enabled($user_id)) {
            return false;
        }
